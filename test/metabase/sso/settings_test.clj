(ns metabase.sso.settings-test
  (:require
   ...
   [metabase.sso.settings.ldap-enabled :as sso.settings.ldap-enabled]
   [metabase.test :as mt]))
      ...
      (mt/with-temporary-setting-values [ldap-enabled false]
        (mt/with-dynamic-fn-redefs [ldap/test-current-ldap-details (constantly {:status :ERROR :message "test error"})]
          (is (thrown-with-msg? clojure.lang.ExceptionInfo
                                ...
                                (sso.settings.ldap-enabled/ldap-enabled! true))))
        (mt/with-dynamic-fn-redefs [ldap/test-current-ldap-details (constantly {:status :SUCCESS})]
          (sso.settings.ldap-enabled/ldap-enabled! true)
          (is (sso.settings.ldap-enabled/ldap-enabled))
          (sso.settings.ldap-enabled/ldap-enabled! false)
          (is (not (sso.settings.ldap-enabled/ldap-enabled))))))))
