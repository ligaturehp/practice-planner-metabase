...
import * as iframe from "metabase/utils/iframe";
...
describe("useTimelineEvents", () => {
  ...
  describe.each([
    ...
    [
      "full-app embeds",
      () => jest.spyOn(iframe, "isWithinIframe").mockReturnValue(true),
    ],
  ])("on %s", (_surface, mockSurface) => {
    it.each([
      ["saved selections", {}],
      ["explicit visibility", { timelineEventsVisibility: SAVED_VISIBILITY }],
      ["explicit events", { timelineEvents: [SHOWN_EVENT] }],
    ])(
      "does not show, request, or report events from %s",
      async (_input, props) => {
        await mockSurface();
        const onTimelineEventsShown = jest.fn();
