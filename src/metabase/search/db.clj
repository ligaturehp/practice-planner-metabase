(ns metabase.search.db
  "Application database queries for search. Keeping queries here isolates database access; callers may still use
  Toucan 2 for connection and transaction management."
  (:require
...
(mu/defn create-search-index-table!
  "Create the search index table named `table-name` on `conn`, with the `columns` the active search engine
  specialization built."
  [conn       :- [:maybe (ms/InstanceOfClass java.sql.Connection)]
   table-name :- [:or :keyword :string]
   columns    :- [:sequential vector?]]
  (t2/query conn {:create-table table-name, :with-columns columns}))

(mu/defn run-search-index-statement!
  "Run a single post-creation SQL statement (e.g. an index creation) for a search index table, on `conn`."
  [conn      :- [:maybe (ms/InstanceOfClass java.sql.Connection)]
...
(mu/defn delete-named-pending-index-metadata!
  "Delete the pending SearchIndexMetadata row of `engine`, `version`, `lang-code`, and `index-name`, on `conn`."
  [conn       :- [:maybe (ms/InstanceOfClass java.sql.Connection)]
   engine     :- :keyword
   version    :- :string
   lang-code  :- :string
...
(mu/defn pending-index-metadata-exists?
  "Whether the pending SearchIndexMetadata row of `engine`, `version`, and `lang-code` exists, read on `conn`."
  [conn      :- [:maybe (ms/InstanceOfClass java.sql.Connection)]
   engine    :- :keyword
   version   :- :string
   lang-code :- :string]
...
(mu/defn named-pending-index-metadata-exists?
  "Whether the pending SearchIndexMetadata row of `engine`, `version`, `lang-code`, and `index-name` exists, read on
  `conn`."
  [conn       :- [:maybe (ms/InstanceOfClass java.sql.Connection)]
   engine     :- :keyword
   version    :- :string
...
(mu/defn activate-named-pending-index-metadata!
  "Activate the pending SearchIndexMetadata row of `engine`, `version`, `lang-code`, and `index-name`, on `conn`."
  [conn       :- [:maybe (ms/InstanceOfClass java.sql.Connection)]
   engine     :- :keyword
   version    :- :string
   lang-code  :- :string
...
(mu/defn lease-times
  "The app database's current time, and the lease expiry `duration-millis` later, read on `conn` without converting
  JDBC time types."
  [conn            :- (ms/InstanceOfClass java.sql.Connection)
   duration-millis :- :int]
  (t2/query-one conn
...
(mu/defn take-over-expired-lease!
  "Take over the lease at `coordinate` on `conn`, if it expired at or before `now`, by writing `claim`. Returns the
...
(mu/defn delete-lease!
  "Delete the lease of `engine`, `version`, and `lang-code` on `conn`, if `owner` still holds it. Returns the number of
  rows deleted."
  [conn      :- (ms/InstanceOfClass java.sql.Connection)
   engine    :- [:maybe :string]
