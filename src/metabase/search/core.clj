...
(defn reindex!
  "Build and activate a new index while continuing to update the current one. By default, runs asynchronously and
  returns a future. Runs synchronously and returns a delivered promise when `search.ingestion/*force-sync*` is true or
  `:async?` is false."
