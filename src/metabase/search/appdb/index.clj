...
(defn create-table!
  ...
  (search.lease/do-with-ddl-connection
   (fn [conn]
     (search.db/create-search-index-table! conn table-name (specialization/table-schema base-schema))
     (let [table-name (name table-name)]
       (doseq [stmt (specialization/post-create-statements table-name table-name)]
         (search.db/run-search-index-statement! conn stmt))))))
...
(defn- safe-batch-upsert!
  ...
  Returns the table name written, or nil if no table is tracked or the batch is skipped. Interruptions and failed
  retries propagate. Stale tracking is refreshed once."
