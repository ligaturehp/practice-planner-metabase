...
(api.macros/defendpoint :get "/root/items"
  ...
  [_route-params
   {:keys [models archived namespace pinned_state sort_column sort_direction official_collections_first
           ...
           q include_available_models show_exploration_documents]} :- [:map
                                                                       ...
                                                                       [:collection_type             {:optional true} children-query/CollectionType]
                                                                       [:archived                    {:default false} [:maybe ms/BooleanValue]]
                                                                       [:namespace                   {:optional true} [:maybe ms/NonBlankString]]
                                                                       [:include_library             {:default false} [:maybe ms/BooleanValue]]
                                                                       [:pinned_state                {:optional true} [:maybe (into [:enum] children-query/valid-pinned-state-values)]]
                                                                       [:sort_column                 {:optional true} [:maybe (into [:enum] children-query/valid-sort-columns)]]
                                                                       [:sort_direction              {:optional true} [:maybe (into [:enum] children-query/valid-sort-directions)]]
  ...
  (let [root-collection (assoc collection/root-collection :namespace namespace)
        ...
        options         {:archived?                   (boolean archived)
                         ...
                         :search-text                 q
                         :sort-info                   {:sort-column                 (or (some-> sort_column children-query/normalize-sort-choice) :name)
                                                       :sort-direction              (or (some-> sort_direction children-query/normalize-sort-choice) :asc)
