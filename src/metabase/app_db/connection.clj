...
(defn gated-data-source
  "Wrap `data-source` so opening a connection takes the current application database's read lock. Use this for
  auxiliary pools over the raw data source so snapshot restores block them too."
...
(defn do-with-independent-connection-transaction
  "Run `f` as a top-level transaction on `connection`, isolated from any ambient transaction. App-db transaction
  nesting is thread-local; without this isolation, Toucan treats the new connection as a nested savepoint and does not
